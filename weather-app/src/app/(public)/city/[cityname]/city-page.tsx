...
import { Hint } from '@/components/hints/hint/hint';
...
function DetailsPage({ cityname }: { cityname: string }) {
  ...
  const city = useSelector((state: { cities: CitiesProps }) =>
    selectCityByName(state, cityname)
  );
    ...
    if (data) {
      if (city) {
        dispatch(
          updateCityWeather({
            ...city,
            weather: data as CityWeatherProps,
  ...
  if (!isClient) {
    return (
      <div className='w-6xl flex flex-col items-center justify-start min-h-screen pr-8 pl-8 pb-8 sm:pb-5 sm:pr-5 sm:pl-5 gap-4 '>
        <div className='w-full font-sans text-xl pb-4 text-black dark:text-zinc-50 bg-surface-light-3 dark:bg-gray-700 p-5 rounded-xl hover:shadow-md '>
          ...Loading
        </div>
      </div>
    );
  ...
  if (!isClient || !city) {
    return (
      <div className='w-6xl min-h-screen flex flex-col items-center text-center px-4 '>
        <div className='w-full pb-4 text-black dark:text-zinc-50 bg-surface-light-3 dark:bg-gray-700 p-5 rounded-xl hover:shadow-md '>
          <h1 className='text-4xl font-noto font-bold mb-4'>City not found</h1>
          <p className='font-sans text-neutral-600 dark:text-neutral-100 mb-6'>
  ...
  return (
    <div className='w-6xl flex flex-col items-center justify-start min-h-screen pr-8 pl-8 pb-8 sm:pb-5 sm:pr-5 sm:pl-5 gap-4 '>
      <div className='w-full flex justify-start '>
        <Link
          href='/'
          className='font-sans text-left mr-3 bg-surface-light-3 dark:bg-gray-700 p-2 rounded-md hover:shadow-md hover:bg-surface-light-2 dark:hover:bg-gray-500'
        >
          Back
        </Link>
        <div className='mr-3 flex hover:shadow-md'>
          <Button
            label='Refresh'
            isRounded
            onClick={() => {
